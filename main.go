			...
			go func(update tg.Update, titles []Title, bot *tg.BotAPI, users, titlesC,
				...
				if strings.HasPrefix(strings.ToLower(txt), "о вомбате") {
					...
					case "кик":
						...
						if kWomb.ID == from {
							replyToMsg(messID, "Если хотите выйти из клана, то напишите `клан выйти`", peer, bot)
							return
						}
						var appmsg string
						var nm []int64
		...
		go func(update tg.Update, titles []Title, bot *tg.BotAPI, users, titlesC,
			...
			if strings.HasPrefix(txt, "/start") {
				...
				if len(args) < 2 {
					...
					if amount, err := strconv.ParseUint(args[0], 10, 64); err == nil {
						...
						if len([]rune(name)) > 64 {
							...
							ctx, bson.M{"name": cins(name)}); err == nil && rCount != 0 {
							var tWomb User
				...
				default:
					replyToMsg(messID,
						fmt.Sprintf(
							"Что такое `%s`?\nP.S. некоторые команды работают только в групповых чатах)",
							args[1],
						),
						peer, bot,
					)
